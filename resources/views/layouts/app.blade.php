...
<nav class="flex py-5 bg-indigo-500 text-white">
    ...
    <ul class="w-1/2 px-16 ml-auto flex justify-end pt-1">
        @if(auth()->check())
            ...
            <li>
                <a href="{{ route('user.logout') }}" class="font-bold
          py-3 px-4 rounded-md bg-red-500 hover:bg-red-600">Log Out</a>
            </li>
        @else
            <li class="mx-6">
                <a href="{{ route('user.toLogin') }}" class="font-semibold
          hover:bg-indigo-700 py-3 px-4 rounded-md">LogIn</a>
            </li>
            <li>
                <a href="{{ route('user.toSignup') }}" class="font-semibold
          border-2 border-white py-2 px-4 rounded-md hover:bg-white
          hover:text-indigo-700">signup</a>
            </li>
        @endif
    </ul>
